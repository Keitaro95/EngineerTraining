...
def create_japanese_bank_account_recognizer():
    ...
    bank_patterns = [
        ...
        Pattern(
            name="jp_bank_full",
            regex=r"\d{4}[\s\-－ー]?\d{3}[\s\-－ー]?\d{7}(?=\D|$)",
            score=0.5
        ),
        # 口座種別＋口座番号：普通1234567、当座1234567
        Pattern(
            name="jp_bank_type_number",
            regex=r"(?:普通|当座)\s*\d{7}(?=\D|$)",
            score=0.6
        ),
        # 口座番号のみ（7桁、contextに依存しない）
        Pattern(
...
def create_japanese_my_number_recognizer():
    ...
    統合正規表現：r"\b\d{12}\b|\b\d{4}[\s\-－ー]\d{4}[\s\-－ー]\d{4}\b"
    """
    my_number_patterns = [
...
# ⑨ クレジットカードセキュリティコード（CVV）
def create_japanese_cvv_recognizer():
    """
    クレジットカードのセキュリティコード（CVV/CVC/CSC）を検出するPatternRecognizer。

    「セキュリティコードは123」のような3桁（または4桁）の数字＋context語のパターン。
    CreditCardRecognizerでは検出できないため、専用PatternRecognizerで対応。

    Visa/MasterCard/JCB: 3桁
    American Express: 4桁
    """
    cvv_patterns = [
        # 3桁のCVV：123
    ...
    return PatternRecognizer(
        ...
        supported_language="ja",
        context=[
            "セキュリティコード", "セキュリティ番号", "CVV", "CVC", "CSC",
            "カード裏面", "裏面の番号", "確認コード", "認証コード"
        ]
    )
...
def get_all_japanese_recognizers():
    ...
    return [
        ...
        create_japanese_cvv_recognizer(),             # ⑨
    ]
